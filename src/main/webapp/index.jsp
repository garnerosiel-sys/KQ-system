  ...
  <div id="app" class="container">
    <div class="sidebar">
      ...
      <nav class="sidebar-nav">
        <ul>
          ...
          <li><a :class="{active: currentModule === 'department'}" @click="switchModule('department')">部门管理</a></li>
          <li><a :class="{active: currentModule === 'rule'}" @click="switchModule('rule')">考勤规则</a></li>
          <li><a :class="{active: currentModule === 'leave'}" @click="switchModule('leave')">请假申请</a></li>
          <li><a :class="{active: currentModule === 'makeup'}" @click="switchModule('makeup')">补卡申请</a></li>
          <li><a :class="{active: currentModule === 'overtime'}" @click="switchModule('overtime')">加班申请</a></li>
          <li><a :class="{active: currentModule === 'workstation'}" @click="switchModule('workstation')">工作台日志</a></li>
          <li><a :class="{active: currentModule === 'user'}" @click="switchModule('user')">用户列表</a></li>
        </ul>
      </nav>
    </div>
      ...
      <div class="content-area">
        <div v-if="currentModule === 'checkin'">
          <div class="layout-grid">
            <div class="card checkin-card">
              <div>上班打卡</div>
              <div class="checkin-status" :style="{color: morningStatus === '已打卡' ? '#2ecc71' : '#e74c3c'}">{{ morningStatus }}</div>
              <button v-if="morningStatus === '未打卡'" class="btn btn-success btn-checkin" @click="handleCheckin('morning')">上班打卡</button>
            </div>
            <div class="card checkin-card">
              <div>下班打卡</div>
              <div class="checkin-status" :style="{color: eveningStatus === '已打卡' ? '#2ecc71' : '#e74c3c'}">{{ eveningStatus }}</div>
              <button v-if="eveningStatus === '未打卡'" class="btn btn-warning btn-checkin" @click="handleCheckin('evening')">下班打卡</button>
            </div>
          </div>
          <div class="card">
            <div class="location-info" v-if="location">{{ location.latitude.toFixed(6) }}, {{ location.longitude.toFixed(6) }}</div>
            <div class="location-info location-warning" v-else-if="!locationLoading">无法获取位置信息</div>
            <div class="location-info" v-else>正在获取位置...</div>
          </div>
          <div class="card">
            <div class="card-title">最近打卡记录</div>
            <div class="record-list">
              <div v-for="record in recentRecords" :key="record.id" class="record-item">
                <span class="record-time">{{ formatDate(record.checkinTime) }}</span>
                <span class="record-type">{{ record.type === 'morning' ? '上班' : '下班' }}</span>
                <span :class="'record-status badge ' + (record.status === '正常' ? 'badge-success' : 'badge-warning')">{{ record.status }}</span>
              </div>
              <div v-if="recentRecords.length === 0" style="text-align:center; padding:20px; color:#999;">暂无打卡记录</div>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="currentModule === 'rule'">
          <div class="card">
            ...
            <table class="table">
              <thead>
                <tr><th>ID</th><th>规则名称</th><th>上班时间</th><th>下班时间</th><th>公司经度</th><th>公司纬度</th><th>允许半径(米)</th><th>创建时间</th><th>操作</th></tr>
              </thead>
              <tbody>
                <tr v-for="rule in rules" :key="rule.id">
                  <td>{{ rule.id }}</td><td>{{ rule.ruleName }}</td><td>{{ rule.workStartTime }}</td><td>{{ rule.workEndTime }}</td><td>{{ rule.centerLongitude }}</td><td>{{ rule.centerLatitude }}</td><td>{{ rule.allowedRadius }}</td><td>{{ formatDate(rule.createTime) }}</td>
                  <td>
    ...
    <div v-if="showRuleModal" class="modal-overlay" @click.self="showRuleModal = false">
      <div class="modal">
        ...
        <div class="form-group"><label>公司经度</label><input v-model="ruleForm.centerLongitude"></div>
        <div class="form-group"><label>公司纬度</label><input v-model="ruleForm.centerLatitude"></div>
        <div class="form-group"><label>允许半径(米)</label><input type="number" v-model="ruleForm.allowedRadius"></div>
        <div class="modal-footer">
